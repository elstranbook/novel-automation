...
"""
    ...
    def generate_promotional_article(self, story_details, article_type='theme_analysis', length_type='medium', 
                                       tone='formal', cta_type='medium', include_links=False):
        """
        Generate promotional articles for marketing YA novels.
        
        Args:
            story_details (dict): The story details including title, genre, theme, etc.
            article_type (str): Type of article to generate
            length_type (str): Length variant (short, medium, long)
            tone (str): Tone preference (formal, casual, warm, emotional)
            cta_type (str): Call-to-action type (soft, medium, strong)
            include_links (bool): Whether to include placeholder links
        ...
        # Format supporting characters for the prompt
        chars_text = ""
        if supporting_chars:
            if isinstance(supporting_chars, list):
                for char in supporting_chars:
                    if isinstance(char, dict):
                        chars_text += f"- {char.get('name', 'Character')}: {char.get('description', '')}\n"
                    else:
                        chars_text += f"- {char}\n"
            else:
                chars_text = str(supporting_chars)
        
        # Define article type descriptions and focus areas
        article_types = {
            'theme_analysis': {
                'name': 'Deep-Dive Theme Article',
                'focus': 'Explore why the themes matter and create emotional connection with readers. Analyze how the themes resonate with teen experiences.',
                'structure': 'Hook → Theme introduction → Theme exploration → Emotional resonance → Reader connection → CTA'
            },
            'character_spotlight': {
                'name': 'Meet the Characters',
                'focus': 'Introduce character backstories, motivations, internal conflicts, and growth arcs. Make readers fall in love with the characters.',
                'structure': 'Hook → Protagonist intro → Character motivations → Internal conflicts → Supporting cast highlights → CTA'
            },
            'author_journey': {
                'name': 'Behind the Scenes / Author Journey',
                'focus': 'Share the writing process, inspiration, challenges faced, and personal connection to the story.',
                'structure': 'Personal hook → Inspiration origin → Writing challenges → Emotional investment → Reader invitation → CTA'
            },
            'world_building': {
                'name': 'World-Building Feature',
                'focus': 'Explore the setting, environment, cultural details, and atmosphere that make the story world come alive.',
                'structure': 'Setting hook → World introduction → Cultural details → Atmosphere → Immersive elements → CTA'
            },
            'quote_spotlight': {
                'name': 'Quote Spotlight',
                'focus': 'Present 10-20 powerful, emotional quotes from the novel with brief commentary on each.',
                'structure': 'Introduction → Categorized quotes (hope, resilience, love, conflict) → Commentary → CTA'
            },
            'lessons_learned': {
                'name': 'Lessons Teens Can Learn',
                'focus': 'Highlight moral lessons and educational value that resonate with teen readers and appeal to parents/educators.',
                'structure': 'Hook about teen struggles → Lesson 1-5 with examples → How book addresses each → Parent/educator appeal → CTA'
            },
            'comparison_article': {
                'name': 'Fans of X Will Love Y',
                'focus': 'Compare to popular YA books, showing how readers who loved similar books will enjoy this one.',
                'structure': 'Hook → Similar book comparisons → Unique elements → Reader testimonial style → CTA'
            },
            'symbolism_article': {
                'name': 'The Hidden Symbolism Article',
                'focus': 'Explain symbols, motifs, foreshadowing, and deeper meanings that enrich the reading experience.',
                'structure': 'Hook about hidden depths → Symbol explanations → Motif analysis → Foreshadowing reveals → Re-read invitation → CTA'
            },
            'emotional_hook': {
                'name': 'Emotional Hook Article',
                'focus': 'Short, moving piece designed to be highly shareable on social media. Pure emotional appeal.',
                'structure': 'Emotional opening → Heart of the story → Emotional peak → Call to experience → CTA'
            },
            'author_letter': {
                'name': 'Author Letter to Readers',
                'focus': 'A heartfelt, personal message from the author to potential readers.',
                'structure': 'Personal greeting → Why I wrote this → What it means to me → My hope for readers → Invitation → CTA'
            },
            'problem_solution': {
                'name': 'Problem-Solution Article',
                'focus': 'Identify teen struggles and show how the book provides insight, comfort, or solutions.',
                'structure': 'Teen struggle identification → How the book addresses it → Character examples → Reader takeaways → CTA'
            },
            'seo_review': {
                'name': 'SEO-Optimized Review-Style Article',
                'focus': 'Long-form, keyword-rich article written in an elegant review style for search engine optimization.',
                'structure': 'SEO title → Hook → Story summary → Character analysis → Theme exploration → Why read now → CTA'
            }
        }
        
        # Get article type info
        article_info = article_types.get(article_type, article_types['theme_analysis'])
        
        # Define length parameters
        length_params = {
            'short': {'words': '300-600', 'description': 'concise, punchy, social media friendly'},
            'medium': {'words': '800-1200', 'description': 'balanced, website-ready, comprehensive'},
            'long': {'words': '1500-2000', 'description': 'detailed, SEO-optimized, blog-ready'}
        }
        
        length_info = length_params.get(length_type, length_params['medium'])
        
        # Define CTA templates
        cta_templates = {
            'soft': f'If "{title}" speaks to your heart, I invite you to experience the journey for yourself.',
            'medium': f'Discover "{title}" today and step into a world of emotion, resilience, and unforgettable characters.',
            'strong': f'Get your copy of "{title}" now and start reading the YA novel readers can\'t stop talking about.'
        }
        
        cta_text = cta_templates.get(cta_type, cta_templates['medium'])
        ...
        **INPUT DETAILS:**
        ...
        Book Summary: {plot_summary}
        
        Target Audience: YA readers aged {target_age}, fans of {genre}
        ...
        {chars_text if chars_text else "Various supporting characters"}
        
        Tone Preference: {tone}
        
        Call-to-Action Type: {cta_type} sell
        Suggested CTA: {cta_text}
        
        Article Length: {length_info['words']} words ({length_info['description']})
        
        Article Style: {article_info['name']}
        Article Focus: {article_info['focus']}
        Suggested Structure: {article_info['structure']}
        ...
        **AVOID:**
        ...
        {"Include placeholder links like [BOOK_LINK] where purchase links should go." if include_links else ""}
        ...
        system_message = f"""You are a bestselling YA book marketing expert and content strategist who creates viral, shareable content.
        Your articles have helped numerous YA novels become bestsellers.
        Write in a {tone} tone that resonates with {target_age} year old readers and their parents/educators.
        ...
        try:
            ...
            max_tokens_map = {'short': 1200, 'medium': 2000, 'long': 3500}
            max_tokens = max_tokens_map.get(length_type, 2000)
            
            # Generate the article
            response = self._ai_completion(prompt, system_message, max_tokens=max_tokens)
            
            # Parse the response to extract title and content
            if isinstance(response, dict) and 'error' in response:
                return {'title': 'Error', 'content': f"Error generating article: {response['error']}"}
            
            # Parse title and content from response
            article_title = title  # Default
            article_content = response
            
            if isinstance(response, str):
                if 'TITLE:' in response and 'ARTICLE:' in response:
                    parts = response.split('ARTICLE:', 1)
                    title_part = parts[0].replace('TITLE:', '').strip()
                    article_title = title_part.split('\n')[0].strip()
                    article_content = parts[1].strip() if len(parts) > 1 else response
            ...
            print(f"Error generating promotional article: {str(e)}")
            return {'title': 'Error', 'content': f"Error generating promotional article: {str(e)}"}
    
    def generate_social_snippets(self, story_details, article_content=None):
        """
        Generate social media snippets for promoting the novel.
        
        Args:
            story_details (dict): The story details
            article_content (str, optional): Existing article to repurpose
            
        Returns:
            dict: Dictionary with snippets for different platforms
        """
        title = self.title
        theme = story_details.get('story_theme', 'Coming of age')
        genre = story_details.get('genre', 'Young Adult Fiction')
        
        context = f"Novel: {title}\nGenre: {genre}\nTheme: {theme}"
        if article_content:
        ...
        system_message = "You are a social media marketing expert specializing in YA book promotion. Create engaging, platform-appropriate content that drives book discovery and purchases."
        
        try:
            response = self._ai_completion(prompt, system_message, max_tokens=2000)
            ...
            return {'content': f"Error generating social snippets: {str(e)}", 'novel_title': title}
...
"""
    ...
    def generate_series_bible(self, series_input):
        """
        Generate a comprehensive Series Bible with all 10 professional components.
        This becomes the law of the story - all content must obey it.
        
        Args:
            series_input (dict): Dictionary containing:
                - title: Series title
                - genre: Genre/subgenre
                - target_age: Target reader age range
                - tone: Tone/vibe of the series
                - setting: World/setting description
                - main_characters: Main character descriptions
                - core_conflict: Central conflict of the series
                - themes: Major themes
                - num_books: Planned number of books (max 5)
        
        Returns:
            dict: Complete Series Bible with all 10 components
        """
        prompt = f"""
You are building a professional SERIES BIBLE for a {self.num_books}-book YA series.
This Bible will become story law - all content must strictly follow it.

**SERIES INPUT:**
Title: {series_input.get('title', self.series_title)}
Genre: {series_input.get('genre', 'Young Adult Fiction')}
Target Age: {series_input.get('target_age', '13-18')}
Tone/Vibe: {series_input.get('tone', 'Emotional, dramatic, hopeful')}
Setting: {series_input.get('setting', 'Contemporary')}
Main Characters: {series_input.get('main_characters', 'To be developed')}
Core Conflict: {series_input.get('core_conflict', 'To be developed')}
Themes: {series_input.get('themes', 'Coming of age, identity, relationships')}
Planned Books: {self.num_books}
...
6) SERIES ARC (One paragraph per book)
   - Book 1 through Book {self.num_books} summaries
...
"""
        ...
        bible = self._ai_completion(
            prompt,
            system_message="""You are an expert YA series architect who creates comprehensive, 
        ...
        # Save to database
        if self.series_id and bible:
            NovelDatabaseService.save_series_bible(self.series_id, bible)
        
        return bible
    
    def generate_series_map(self, series_bible=None):
        """
        Generate a book-by-book map for the entire series.
        Each book entry includes conflict, emotional journey, growth, twists, and foreshadowing.
        
        Args:
            series_bible (dict, optional): The series bible to use as reference
            
        Returns:
            list: List of book maps for each book in the series
        """
        # Get bible from database if not provided
        if not series_bible and self.series_id:
            series_bible = NovelDatabaseService.get_series_bible(self.series_id)
        
        bible_context = ""
        if series_bible:
            bible_context = f"""
SERIES BIBLE REFERENCE:
World Overview: {series_bible.get('world_overview', 'N/A')}
Core Themes: {json.dumps(series_bible.get('themes_symbols', []))}
Character Files: {json.dumps(series_bible.get('character_files', {}))}
Series Arc Summary: {series_bible.get('series_arc_summary', 'N/A')}
"""
        
        prompt = f"""
Create a detailed BOOK-BY-BOOK MAP for a {self.num_books}-book YA series titled "{self.series_title}".
{bible_context}

For EACH of the {self.num_books} books, provide:
...
"""
        ...
        book_maps = self._ai_completion(
            prompt,
            system_message="""You are an expert series planner. Each book must escalate stakes, 
        ...
        # Save each book map to database
        if self.series_id and book_maps:
            if isinstance(book_maps, list):
                for book_map in book_maps:
                    book_num = book_map.get('book_number', 1)
                    NovelDatabaseService.save_series_book_map(self.series_id, book_num, book_map)
        
        return book_maps
    
    def generate_character_evolution(self, characters, series_bible=None):
        """
        Generate detailed character evolution tracking across the entire series.
        
        Args:
            characters (list): List of character names or character dictionaries
            series_bible (dict, optional): Series bible for context
            
        Returns:
            dict: Character evolution data for each character
        """
        # Get bible from database if not provided
        if not series_bible and self.series_id:
            series_bible = NovelDatabaseService.get_series_bible(self.series_id)
        ...
        char_context = ""
        if series_bible and series_bible.get('character_files'):
            char_context = f"CHARACTER FILES FROM BIBLE:\n{json.dumps(series_bible.get('character_files', {}), indent=2)}"
        ...
        if isinstance(characters, list):
            if all(isinstance(c, str) for c in characters):
                ...
                char_list = ", ".join([c.get('name', str(c)) for c in characters])
        else:
            char_list = str(characters)
        
        prompt = f"""
Create a CHARACTER EVOLUTION ENGINE for the following characters in a {self.num_books}-book YA series:
...
7) BOOK-BY-BOOK STATE (for each of {self.num_books} books)
...
"""
        ...
        evolution = self._ai_completion(
            prompt,
            system_message="""You are a master character psychologist. You understand the deep 
        ...
        # Save each character evolution to database
        if self.series_id and evolution:
            if isinstance(evolution, dict):
                for char_name, char_data in evolution.items():
                    if char_name != 'error':
                        NovelDatabaseService.save_character_evolution(
                            self.series_id, char_name, char_data
                        )
        
        return evolution
    
    def generate_book_blueprint(self, book_number, series_bible=None, book_map=None, character_evolution=None):
        """
        Generate a detailed book blueprint/outline for a specific book.
        
        Args:
            book_number (int): Which book in the series (1-5)
            series_bible (dict, optional): Series bible for context
            book_map (dict, optional): Book map for this book
            character_evolution (dict, optional): Character evolution data
            
        Returns:
            dict: Complete book blueprint
        """
        ...
        if self.series_id:
            if not series_bible:
                series_bible = NovelDatabaseService.get_series_bible(self.series_id)
            if not book_map:
                book_map = NovelDatabaseService.get_series_book_map(self.series_id, book_number)
            if not character_evolution:
                character_evolution = NovelDatabaseService.get_character_evolution(self.series_id)
        
        context_parts = []
        if series_bible:
            context_parts.append(f"SERIES BIBLE:\n{json.dumps(series_bible, indent=2)[:2000]}")
        if book_map:
            context_parts.append(f"BOOK MAP:\n{json.dumps(book_map, indent=2)}")
        if character_evolution:
            context_parts.append(f"CHARACTER EVOLUTION:\n{json.dumps(character_evolution, indent=2)[:2000]}")
        
        context = "\n\n".join(context_parts)
        
        prompt = f"""
Generate a detailed BOOK BLUEPRINT for Book {book_number} of {self.num_books} in the "{self.series_title}" series.
...
"""
        ...
        blueprint = self._ai_completion(
            prompt,
            system_message="""You are an expert YA novel architect. Create emotionally powerful 
        ...
        # Save to database
        if self.series_id and blueprint:
            NovelDatabaseService.save_book_blueprint(self.series_id, book_number, blueprint)
        
        return blueprint
    
    def audit_continuity(self, content, book_number=None, chapter_number=None, content_type='chapter'):
        """
        Audit content against the Series Bible for continuity errors.
        
        Args:
            content (str): The content to audit (chapter text, scene, etc.)
            book_number (int, optional): Which book
            chapter_number (int, optional): Which chapter
            content_type (str): Type of content being audited
            
        Returns:
            dict: Audit results with issues and fixes
        """
        # Get series bible for reference
        series_bible = None
        character_evolution = None
        if self.series_id:
            series_bible = NovelDatabaseService.get_series_bible(self.series_id)
            character_evolution = NovelDatabaseService.get_character_evolution(self.series_id)
        ...
        if series_bible:
            bible_context = f"""
SERIES BIBLE TO CHECK AGAINST:
World Rules: {series_bible.get('world_rules', 'N/A')}
Story Rules: {json.dumps(series_bible.get('story_rules', []))}
Continuity Lockfile: {json.dumps(series_bible.get('continuity_lockfile', []))}
Character Files: {json.dumps(series_bible.get('character_files', {}), indent=2)[:1500]}
"""
        
        char_context = ""
        if character_evolution:
            char_context = f"\nCHARACTER EVOLUTION DATA:\n{json.dumps(character_evolution, indent=2)[:1500]}"
        
        prompt = f"""
You are the CONTINUITY ENFORCER for a YA series.

Audit the following {content_type} content against the Series Bible:

CONTENT TO AUDIT (Book {book_number or 'N/A'}, Chapter {chapter_number or 'N/A'}):
---
{content[:3000]}
---
...
1) RULE BREAKS
   ...
   - Breaking established story rules
...
3) CHARACTER DRIFT
   ...
   - Dialogue that doesn't match voice
...
5) EMOTIONAL ERRORS
   ...
   - Mood whiplash
   - Character reactions that don't fit
...
Return as JSON:
{{
  "audit_type": "{content_type}",
  "severity": "minor|moderate|severe",
  "issues_found": [
    {{
      "type": "rule_break|timeline_error|character_drift|logic_gap|emotional_error",
      "description": "What's wrong",
      "location": "Where in the content",
      "why_it_breaks": "Why this is a problem",
      "suggested_fix": "How to fix it"
    }}
  ],
  "overall_assessment": "Summary of continuity status",
  "fixes_applied": []
}}
"""
        
        audit_result = self._ai_completion(
            prompt,
            system_message="""You are a meticulous continuity editor. Find every error, 
            inconsistency, and break from the established series bible. Be thorough but fair.
            Only flag genuine issues, not stylistic preferences.""",
            json_response=True,
            max_tokens=3000
        )
        
        # Add context to audit result
        if audit_result:
            audit_result['book_number'] = book_number
            audit_result['chapter_number'] = chapter_number
            
            # Save to database
            if self.series_id:
                NovelDatabaseService.save_continuity_audit(self.series_id, audit_result)
        
        return audit_result
    
    def update_series_state(self, book_number, book_content_summary=None):
        """
        Update the series state after completing a book.
        This updates character evolution, world state, and the series bible.
        
        Args:
            book_number (int): The book that was just completed
            book_content_summary (str, optional): Summary of what happened in the book
            
        Returns:
            dict: Updated series state
        """
        # Get current state
        series_bible = None
        character_evolution = None
        ...
        context = ""
        if series_bible:
            context += f"\nCURRENT SERIES BIBLE:\n{json.dumps(series_bible, indent=2)[:2000]}"
        if character_evolution:
            context += f"\nCURRENT CHARACTER EVOLUTION:\n{json.dumps(character_evolution, indent=2)[:2000]}"
        
        prompt = f"""
Book {book_number} of the "{self.series_title}" series has been completed.

{f"BOOK SUMMARY: {book_content_summary}" if book_content_summary else ""}

{context}

As the SERIES STATE MEMORY MANAGER, update the following after this book:

1) CHARACTER EVOLUTION UPDATES
   - How has each character changed?
   - New wounds or healing?
   - Shifts in goals or fears?

2) WORLD DAMAGE
   - What has changed in the world?
   - Places destroyed or transformed?
   - Power structures altered?
...
5) NEW SECRETS
   - What new secrets were revealed?
   - What new secrets were created?
   - What remains hidden?

6) LOSSES
   - Character deaths?
   - Lost abilities or resources?
   - Emotional losses?

7) BIBLE UPDATES
   - What needs to be added to the Series Bible?
   - New rules established?
   - New continuity locks?

Return as JSON:
{{
  "character_evolution_updates": {{
    "Character Name": "How they changed in this book"
  }},
  "world_damage": "Description of world changes",
  "power_shifts": "Who gained/lost power",
  "relationship_changes": "Key relationship updates",
  "new_secrets": ["secret1", "secret2"],
  "losses": ["loss1", "loss2"],
  "bible_updates": {{
    "new_rules": ["rule"],
    "new_continuity_locks": ["lock"],
    "updated_mysteries": ["mystery solved or deepened"]
  }}
}}
"""
        
        state_update = self._ai_completion(
            prompt,
            system_message="""You are the series memory keeper. Track every change, every loss, 
            every growth. Nothing resets between books. Characters carry their trauma and growth 
            forward. The world remembers what happened.""",
            json_response=True,
            max_tokens=4000
        )
        
        # Save state update to database
        if self.series_id and state_update:
            NovelDatabaseService.save_series_state(self.series_id, book_number, state_update)
            
            # Update character evolution with new data
            if state_update.get('character_evolution_updates'):
                for char_name, update in state_update['character_evolution_updates'].items():
                    existing = NovelDatabaseService.get_character_evolution(self.series_id, char_name)
                    if existing:
                        # Update book_states with new book data
                        book_states = existing.get('book_states', {})
                        book_states[str(book_number)] = {"summary": update}
                        existing['book_states'] = book_states
                        NovelDatabaseService.save_character_evolution(
                            self.series_id, char_name, existing
                        )
            
            # Update bible with new info
            if state_update.get('bible_updates') and series_bible:
                bible_updates = state_update['bible_updates']
                
                # Add new rules
                if bible_updates.get('new_rules'):
                    current_rules = series_bible.get('story_rules', [])
                    if isinstance(current_rules, list):
                        current_rules.extend(bible_updates['new_rules'])
                        series_bible['story_rules'] = current_rules
                
                # Add new continuity locks
                if bible_updates.get('new_continuity_locks'):
                    current_locks = series_bible.get('continuity_lockfile', [])
                    if isinstance(current_locks, list):
                        current_locks.extend(bible_updates['new_continuity_locks'])
                        series_bible['continuity_lockfile'] = current_locks
                
                NovelDatabaseService.save_series_bible(self.series_id, series_bible)
