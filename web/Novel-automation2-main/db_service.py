...
class NovelDatabaseService:
    ...
    def _get_connection(cls):
        ...
        # Create series_bible table - comprehensive series bible with all 10 components
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS series_bible (
            ...
            world_rules TEXT,
            history_lore TEXT,
            character_files TEXT,
            relationship_map TEXT,
            series_arc_summary TEXT,
            themes_symbols TEXT,
            story_rules TEXT,
            continuity_lockfile TEXT,
            unanswered_mysteries TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        ...
        # Create series_book_map table - detailed book-by-book planning
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS series_book_map (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            series_id INTEGER NOT NULL,
        ...
        # Create character_evolution table - track character growth across series
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS character_evolution (
            ...
            wounds TEXT,
            fears TEXT,
            internal_battle TEXT,
            corruption_or_healing TEXT,
            end_of_series_form TEXT,
            book_states TEXT,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (series_id) REFERENCES series (id) ON DELETE CASCADE
        ...
        # Create book_blueprint table - detailed book outlines
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS book_blueprint (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            series_id INTEGER NOT NULL,
            book_number INTEGER NOT NULL,
            opening_shift TEXT,
        ...
        # Create continuity_audit table - track audits and fixes
        cursor.execute('''
        ...
        # Create series_state table - track state changes after each book
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS series_state (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            series_id INTEGER NOT NULL,
            after_book_number INTEGER NOT NULL,
            character_evolution_updates TEXT,
            world_damage TEXT,
            power_shifts TEXT,
            relationship_changes TEXT,
            new_secrets TEXT,
        ...
        # Create table for promotional articles
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS promotional_articles (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            novel_id INTEGER NOT NULL,
    ...
    # ============================================
    # SERIES BIBLE METHODS
    # ============================================
    
    @classmethod
    def save_series_bible(cls, series_id, bible_data):
        """
        Save or update the comprehensive series bible
        
        Args:
            series_id (int): The series ID
            bible_data (dict): Dictionary containing all 10 bible components
        ...
        try:
            ...
            cursor.execute("SELECT id FROM series_bible WHERE series_id = ?", (series_id,))
            existing = cursor.fetchone()
            
            if existing:
                cursor.execute("""
                    UPDATE series_bible SET 
                        world_overview = ?, world_rules = ?, history_lore = ?,
                        character_files = ?, relationship_map = ?, series_arc_summary = ?,
                        themes_symbols = ?, story_rules = ?, continuity_lockfile = ?,
                        unanswered_mysteries = ?, updated_at = CURRENT_TIMESTAMP
                    WHERE series_id = ?
                """, (
                    bible_data.get('world_overview', ''),
                    bible_data.get('world_rules', ''),
                    bible_data.get('history_lore', ''),
                    json.dumps(bible_data.get('character_files', {})),
                    json.dumps(bible_data.get('relationship_map', {})),
                    bible_data.get('series_arc_summary', ''),
                    json.dumps(bible_data.get('themes_symbols', [])),
                    json.dumps(bible_data.get('story_rules', [])),
                    json.dumps(bible_data.get('continuity_lockfile', [])),
                    json.dumps(bible_data.get('unanswered_mysteries', [])),
                    series_id
                ))
            else:
                cursor.execute("""
                    INSERT INTO series_bible 
                        (series_id, world_overview, world_rules, history_lore,
                         character_files, relationship_map, series_arc_summary,
                         themes_symbols, story_rules, continuity_lockfile, unanswered_mysteries)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    series_id,
                    bible_data.get('world_overview', ''),
            ...
            print(f"Database error saving series bible: {str(e)}")
            return False
    
    @classmethod
    def get_series_bible(cls, series_id):
        """
        Get the comprehensive series bible
        
        Args:
            series_id (int): The series ID
            
        Returns:
            dict: Series bible data, or None if not found
        """
        try:
            conn = cls._get_connection()
            cursor = conn.cursor()
            
            cursor.execute("SELECT * FROM series_bible WHERE series_id = ?", (series_id,))
            result = cursor.fetchone()
            conn.close()
            ...
            if result:
                bible = dict(result)
                # Parse JSON fields
                for field in ['character_files', 'relationship_map', 'themes_symbols', 
                             'story_rules', 'continuity_lockfile', 'unanswered_mysteries']:
                    if bible.get(field):
                        try:
                            bible[field] = json.loads(bible[field])
                        except:
                            pass
                return bible
            return None
        except Exception as e:
            print(f"Database error getting series bible: {str(e)}")
            return None
    
    @classmethod
    def save_series_book_map(cls, series_id, book_number, book_map_data):
        """
        Save the book-by-book planning map for a specific book
        
        Args:
            series_id (int): The series ID
            book_number (int): Book position in series
            book_map_data (dict): Book planning data
        ...
        try:
            ...
            # Check if exists
            cursor.execute("SELECT id FROM series_book_map WHERE series_id = ? AND book_number = ?", 
                          (series_id, book_number))
            existing = cursor.fetchone()
            
            if existing:
                cursor.execute("""
                    UPDATE series_book_map SET 
                        central_conflict = ?, emotional_journey = ?, character_growth = ?,
                        twist_reveal = ?, final_state = ?, foreshadowing_seeds = ?,
                        stakes_escalation = ?
                    WHERE series_id = ? AND book_number = ?
                """, (
                    book_map_data.get('central_conflict', ''),
                    book_map_data.get('emotional_journey', ''),
                    book_map_data.get('character_growth', ''),
                    book_map_data.get('twist_reveal', ''),
                    book_map_data.get('final_state', ''),
                    json.dumps(book_map_data.get('foreshadowing_seeds', [])),
                    book_map_data.get('stakes_escalation', ''),
                    series_id, book_number
                ))
            else:
                cursor.execute("""
                    INSERT INTO series_book_map 
                        (series_id, book_number, central_conflict, emotional_journey,
                         character_growth, twist_reveal, final_state, foreshadowing_seeds, stakes_escalation)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    series_id, book_number,
                    book_map_data.get('central_conflict', ''),
            ...
            print(f"Database error saving series book map: {str(e)}")
            return False
    
    @classmethod
    def get_series_book_map(cls, series_id, book_number=None):
        """
        Get book map for a series or specific book
        
        Args:
            series_id (int): The series ID
            book_number (int, optional): Specific book number
            
        Returns:
            dict or list: Book map data
        """
        try:
            conn = cls._get_connection()
            cursor = conn.cursor()
            ...
            if book_number:
                cursor.execute("SELECT * FROM series_book_map WHERE series_id = ? AND book_number = ?",
                              (series_id, book_number))
                result = cursor.fetchone()
                conn.close()
                ...
                if result:
                    data = dict(result)
                    if data.get('foreshadowing_seeds'):
                        try:
                            data['foreshadowing_seeds'] = json.loads(data['foreshadowing_seeds'])
                        except:
                            pass
                    return data
                return None
            else:
                cursor.execute("SELECT * FROM series_book_map WHERE series_id = ? ORDER BY book_number",
                              (series_id,))
            ...
            print(f"Database error getting series book map: {str(e)}")
            return None if book_number else []
    
    @classmethod
    def save_character_evolution(cls, series_id, character_name, evolution_data):
        """
        Save character evolution tracking data
        
        Args:
            series_id (int): The series ID
            character_name (str): Character name
            evolution_data (dict): Evolution tracking data
        ...
        try:
            ...
            # Check if exists
            cursor.execute("SELECT id FROM character_evolution WHERE series_id = ? AND character_name = ?",
                          (series_id, character_name))
            existing = cursor.fetchone()
            
            if existing:
                cursor.execute("""
                    UPDATE character_evolution SET 
                        starting_state = ?, wounds = ?, fears = ?,
                        internal_battle = ?, corruption_or_healing = ?,
                        end_of_series_form = ?, book_states = ?,
                        updated_at = CURRENT_TIMESTAMP
                    WHERE series_id = ? AND character_name = ?
                """, (
                    evolution_data.get('starting_state', ''),
                    evolution_data.get('wounds', ''),
                    evolution_data.get('fears', ''),
                    evolution_data.get('internal_battle', ''),
                    evolution_data.get('corruption_or_healing', ''),
                    evolution_data.get('end_of_series_form', ''),
                    json.dumps(evolution_data.get('book_states', {})),
                    series_id, character_name
                ))
            else:
                cursor.execute("""
                    INSERT INTO character_evolution 
                        (series_id, character_name, starting_state, wounds, fears,
                         internal_battle, corruption_or_healing, end_of_series_form, book_states)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    series_id, character_name,
                    evolution_data.get('starting_state', ''),
            ...
            print(f"Database error saving character evolution: {str(e)}")
            return False
    
    @classmethod
    def get_character_evolution(cls, series_id, character_name=None):
        """
        Get character evolution data
        
        Args:
            series_id (int): The series ID
            character_name (str, optional): Specific character
            
        Returns:
            dict or list: Character evolution data
        """
        try:
            conn = cls._get_connection()
            cursor = conn.cursor()
            ...
            if character_name:
                cursor.execute("SELECT * FROM character_evolution WHERE series_id = ? AND character_name = ?",
                              (series_id, character_name))
                result = cursor.fetchone()
                conn.close()
                ...
                if result:
                    data = dict(result)
                    if data.get('book_states'):
                        try:
                            data['book_states'] = json.loads(data['book_states'])
                        except:
                            pass
                    return data
                return None
            else:
                cursor.execute("SELECT * FROM character_evolution WHERE series_id = ?", (series_id,))
                results = cursor.fetchall()
                conn.close()
            ...
            print(f"Database error getting character evolution: {str(e)}")
            return None if character_name else []
    
    @classmethod
    def save_book_blueprint(cls, series_id, book_number, blueprint_data):
        """
        Save detailed book blueprint/outline
        
        Args:
            series_id (int): The series ID
            book_number (int): Book position
            blueprint_data (dict): Blueprint data
        ...
        try:
            ...
            # Check if exists
            cursor.execute("SELECT id FROM book_blueprint WHERE series_id = ? AND book_number = ?",
                          (series_id, book_number))
            existing = cursor.fetchone()
            
            if existing:
                cursor.execute("""
                    UPDATE book_blueprint SET 
                        opening_shift = ?, midpoint_shock = ?, lowest_point = ?,
                        climax = ?, ending_change = ?, relationship_changes = ?,
                        theme_pressure = ?, next_book_setup = ?, full_outline = ?
                    WHERE series_id = ? AND book_number = ?
                """, (
                    blueprint_data.get('opening_shift', ''),
                    blueprint_data.get('midpoint_shock', ''),
                    blueprint_data.get('lowest_point', ''),
                    blueprint_data.get('climax', ''),
                    blueprint_data.get('ending_change', ''),
                    blueprint_data.get('relationship_changes', ''),
                    blueprint_data.get('theme_pressure', ''),
                    blueprint_data.get('next_book_setup', ''),
                    blueprint_data.get('full_outline', ''),
                    series_id, book_number
                ))
            else:
                cursor.execute("""
                    INSERT INTO book_blueprint 
                        (series_id, book_number, opening_shift, midpoint_shock, lowest_point,
                         climax, ending_change, relationship_changes, theme_pressure,
                         next_book_setup, full_outline)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    series_id, book_number,
                    blueprint_data.get('opening_shift', ''),
            ...
            print(f"Database error saving book blueprint: {str(e)}")
            return False
    
    @classmethod
    def get_book_blueprint(cls, series_id, book_number):
        """
        Get book blueprint
        ...
        try:
            ...
            cursor.execute("SELECT * FROM book_blueprint WHERE series_id = ? AND book_number = ?",
                          (series_id, book_number))
            result = cursor.fetchone()
            conn.close()
            ...
            print(f"Database error getting book blueprint: {str(e)}")
            return None
    
    @classmethod
    def save_continuity_audit(cls, series_id, audit_data):
        """
        Save a continuity audit record
        
        Args:
            series_id (int): The series ID
            audit_data (dict): Audit results
            
        Returns:
            int: Audit ID or None
        """
        try:
            conn = cls._get_connection()
            cursor = conn.cursor()
            ...
            cursor.execute("""
                INSERT INTO continuity_audit 
                    (series_id, book_number, chapter_number, audit_type, 
                     issues_found, fixes_applied, severity)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                series_id,
                audit_data.get('book_number'),
                audit_data.get('chapter_number'),
                audit_data.get('audit_type', 'general'),
                json.dumps(audit_data.get('issues_found', [])),
                json.dumps(audit_data.get('fixes_applied', [])),
                audit_data.get('severity', 'minor')
            ))
            ...
            print(f"Database error saving continuity audit: {str(e)}")
            return None
    
    @classmethod
    def get_continuity_audits(cls, series_id, book_number=None):
        """
        Get continuity audit history
        
        Args:
            series_id (int): The series ID
            book_number (int, optional): Filter by book
        ...
        try:
            ...
            for result in results:
                data = dict(result)
                for field in ['issues_found', 'fixes_applied']:
                    if data.get(field):
                        try:
            ...
            print(f"Database error getting continuity audits: {str(e)}")
            return []
    
    @classmethod
    def save_series_state(cls, series_id, after_book_number, state_data):
        """
        Save series state after a book completion
        
        Args:
            series_id (int): The series ID
            after_book_number (int): Book number just completed
            state_data (dict): State changes data
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO series_state 
                    (series_id, after_book_number, character_evolution_updates,
                     world_damage, power_shifts, relationship_changes,
                     new_secrets, losses, bible_updates)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                series_id, after_book_number,
                json.dumps(state_data.get('character_evolution_updates', {})),
                state_data.get('world_damage', ''),
                state_data.get('power_shifts', ''),
                state_data.get('relationship_changes', ''),
                json.dumps(state_data.get('new_secrets', [])),
                json.dumps(state_data.get('losses', [])),
                json.dumps(state_data.get('bible_updates', {}))
            ))
            ...
            print(f"Database error saving series state: {str(e)}")
            return False
    
    @classmethod
    def get_series_state(cls, series_id, after_book_number=None):
        """
        Get series state history
        
        Args:
            series_id (int): The series ID
            after_book_number (int, optional): Specific book
            
        Returns:
            dict or list: State data
        """
        try:
            conn = cls._get_connection()
            cursor = conn.cursor()
            
            if after_book_number:
                cursor.execute("""
                    SELECT * FROM series_state 
                    WHERE series_id = ? AND after_book_number = ?
                """, (series_id, after_book_number))
                result = cursor.fetchone()
                conn.close()
                ...
                if result:
                    data = dict(result)
                    for field in ['character_evolution_updates', 'new_secrets', 'losses', 'bible_updates']:
                        if data.get(field):
                            try:
                                data[field] = json.loads(data[field])
                            except:
            ...
            print(f"Database error getting series state: {str(e)}")
            return None if after_book_number else []
    ...
    @classmethod
    def save_promotional_article(cls, novel_id, article_type, content, length_type='medium', tone='formal', cta_type='medium', title=None):
        """
        Save a promotional article for marketing purposes
        
        Args:
            novel_id (int): The novel ID
            article_type (str): Type of article (theme_analysis, character_spotlight, etc.)
            content (str): The article content
            length_type (str): Length variant (short, medium, long)
            tone (str): Tone preference (formal, casual, warm)
            cta_type (str): Call-to-action type (soft, medium, strong)
            title (str, optional): Article title
            
        Returns:
            int: The ID of the saved article
        """
        try:
            conn = cls._get_connection()
            cursor = conn.cursor()
            ...
            # Insert new article
            cursor.execute(
                "INSERT INTO promotional_articles (novel_id, article_type, length_type, tone, cta_type, content, title) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (novel_id, article_type, length_type, tone, cta_type, content, title)
            )
            ...
            print(f"Database error saving promotional article: {str(e)}")
            return None
            
    @classmethod
    def get_promotional_articles(cls, novel_id, article_type=None):
        """
        Get promotional articles for a novel
        
        Args:
            novel_id (int): The novel ID
            article_type (str, optional): Filter by article type
        ...
        try:
            ...
            if article_type:
                cursor.execute(
                    "SELECT id, article_type, length_type, tone, cta_type, content, title, created_at FROM promotional_articles WHERE novel_id = ? AND article_type = ? ORDER BY created_at DESC",
                    (novel_id, article_type)
                )
            else:
                cursor.execute(
                    "SELECT id, article_type, length_type, tone, cta_type, content, title, created_at FROM promotional_articles WHERE novel_id = ? ORDER BY created_at DESC",
                    (novel_id,)
                )
            ...
            print(f"Database error getting promotional articles: {str(e)}")
            return []
            
    @classmethod
    def delete_promotional_article(cls, article_id):
        """
        Delete a promotional article
        ...
        try:
            ...
            cursor.execute("DELETE FROM promotional_articles WHERE id = ?", (article_id,))
            ...
            print(f"Database error deleting promotional article: {str(e)}")
            return False
