...
function isAllowedUrl(url: string): boolean {
  try {
    ...
    // Allow proxying any HTTPS public host; block private/local hosts.
    // This avoids false negatives when template assets are on new CDN domains.
...
export async function GET(request: NextRequest) {
  ...
  if (!isAllowedUrl(url)) {
    ...
    console.warn('[image-proxy] Hostname not allowed:', host);
