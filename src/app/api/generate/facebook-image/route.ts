...
export async function POST(req: Request) {
  try {
    ...
    console.log("📱 Generating Facebook promotional image:", { model, promptLength: prompt?.length, novelId });
...
- Dramatically improve visual storytelling for social media
- Add cinematic atmosphere and emotional depth
- Optimize for 4:5 vertical format (Facebook feed)
...
- Leave room for headline text overlay
...
- Create strong bold focal points
...
- Optimize for vertical 4:5 social media format
- Ensure the composition leaves negative space for text overlays
...
- Make it highly descriptive but coherent`,
        prompt: `Here is the original prompt to reimagine for a Facebook promotional image:\n\n${prompt}`,
        jsonResponse: false,
        maxTokens: 1500,
      });
      ...
      if (reimagined && typeof reimagined === "string" && reimagined.trim().length > 50) {
        ...
        console.log("✅ Facebook prompt reimagined successfully. Enhanced length:", enhancedPrompt.length);
      } else {
    ...
    // 2. Wrap the enhanced prompt with Facebook style requirements
    const imagePrompt = `Create a professional Facebook promotional image in 4:5 format for a bestselling novel campaign.
...
- premium modern design`;
    ...
    if (model.includes("gpt-image-2")) {
      // gpt-image-2 supports custom sizes
    ...
    if (novelId && imageBuffer) {
      ...
      if (bucketReady) {
        try {
          const fileName = `${novelId}/facebook_${Date.now()}.png`;
          ...
          if (uploadError) {
            ...
            console.log("✅ Facebook image uploaded to storage:", publicUrl);
          }
        } catch (storageError) {
      ...
      // 6. Save Facebook image record to database
      if (!finalUrl) {
        finalUrl = `data:image/png;base64,${rawBase64}`;
        console.warn("⚠️ No persistent URL available — Facebook image will display temporarily but won't persist after refresh");
      }
      ...
      try {
        ...
        if (userId && storageUploadSucceeded && finalUrl) {
          ...
          if (insertError) {
            console.error("❌ Error inserting Facebook image record:", insertError.message);
          } else {
            console.log("✅ Facebook image record saved to database for novel:", novelId);
          }
        }
      } catch (dbError) {
    ...
    console.error("❌ Facebook image generation error:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to generate Facebook image" },
      { status: 500 }
    );
  }
}
