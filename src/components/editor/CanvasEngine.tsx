...
export const CanvasEngine = forwardRef<CanvasEngineHandle, CanvasEngineProps>(({
    ...
    if (userImageRef.current && userImage) {
      ...
      if (smartObjectLayer) {
        ...
        if (perspective) {
          ...
          // Placeit-style print realism: re-apply local lighting from the base photo
          // onto the warped cover so it feels printed into the scene.
          if (baseImageRef.current) {
            applyPrintedInkIntegrationWithQuad(ctx, baseImageRef.current, dst);
          }
        } else {
          ...
          if (baseImageRef.current) {
            applyPrintedInkIntegrationWithRect(
              ctx,
  ...
  useEffect(() => {
    ...
    const candidateUrls = Array.from(
      new Set(
        [template.baseImage, template.thumbnail]
          .filter((value): value is string => !!value)
          .flatMap((value) => {
            const proxied = proxyImageUrl(value);
            return proxied && proxied !== value ? [proxied, value] : [value];
          })
      )
    );
    ...
    console.log('[CanvasEngine] Loading base image:', {
      ...
      proxied: (primaryUrl || '').substring(0, 100),
      candidates: candidateUrls.map((u) => u.substring(0, 100)),
    ...
    img.onerror = () => {
      const failedUrl = candidateUrls[currentCandidateIndex];
      currentCandidateIndex += 1;
      if (currentCandidateIndex < candidateUrls.length) {
        const nextUrl = candidateUrls[currentCandidateIndex];
        console.warn('[CanvasEngine] Base image failed, trying next candidate:', (failedUrl || '').substring(0, 80), '->', (nextUrl || '').substring(0, 80));
        img.src = nextUrl;
        return;
      }
      console.warn('[CanvasEngine] Base image failed to load from all candidates:', candidateUrls.map((u) => u.substring(0, 80)));
            ...
            if (perspective) {
              ...
              if (baseImageRef.current) {
                applyPrintedInkIntegrationWithRect(
                  ...
                  bounds.x * hiScaleX,
                  bounds.y * hiScaleY,
                  bounds.width * hiScaleX,
                  bounds.height * hiScaleY
                );
              }
              ctx.restore();
            }
...
function applyPrintedInkIntegrationWithQuad(
  ctx: CanvasRenderingContext2D,
  ...
  // Multiply pass carries shadow/texture imprint.
  ctx.globalCompositeOperation = 'multiply';
  ctx.globalAlpha = 0.22;
  ctx.drawImage(baseImage, 0, 0, ctx.canvas.width, ctx.canvas.height);

  // Screen pass restores subtle highlight sheen (paper/gloss behavior).
...
function applyPrintedInkIntegrationWithRect(
  ctx: CanvasRenderingContext2D,
  baseImage: HTMLImageElement,
  x: number,
  y: number,
